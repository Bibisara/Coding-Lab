<?php 
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($user_name) && !empty($password) && !empty($username) && !empty($user_surname) && !empty($user_email)  && !is_numeric($username))
		{

			//save to database
			$user_id = random_num(20);
			$query = "insert into users (user_id,username,password,user_name,user_surname,user_email,user_type) values ('$user_id','$username','$password','$user_name','$user_surname','$user_email','$user_type')";

			mysqli_query($conn, $query);

			header("Location: psychologypage.html");
			die;
		}else
		{
